...
void openmp_simd_loopUnroll_registerBlock(uint64_t* restrict result,
    ...
    for (int i = 0; i < WIDTH2; i++)
    {
        for (int m = 0; m < WIDTH1; m++)
        {
			__m256i m1_0 = _mm256_set1_epi32(((uint32_t)(matrix1[m*(WIDTH1+PAD1)])));
			__m256i m1_1 = _mm256_set1_epi32(((uint32_t)(matrix1[m*(WIDTH1+PAD1)+1])));
			__m256i m1_2 = _mm256_set1_epi32(((uint32_t)(matrix1[m*(WIDTH1+PAD1)+2])));
			__m256i m1_3 = _mm256_set1_epi32(((uint32_t)(matrix1[m*(WIDTH1+PAD1)+3])));
			__m256i m1_4 = _mm256_set1_epi32(((uint32_t)(matrix1[m*(WIDTH1+PAD1)+4])));
			__m256i m1_5 = _mm256_set1_epi32(((uint32_t)(matrix1[m*(WIDTH1+PAD1)+5])));
			__m256i m1_6 = _mm256_set1_epi32(((uint32_t)(matrix1[m*(WIDTH1+PAD1)+6])));
